...
class PlayState extends MusicBeatState
{
	...
	var glitch = new FlxSprite(-600, -200).loadGraphic(Paths.image('glitcherBack'));
	// API stuff
	...
	override public function create()
	{
						...
						case 'morning':
							{
								defaultCamZoom = 0.9;
								curStage = 'morning';
								var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('morningBack'));
								bg.antialiasing = true;
								bg.scrollFactor.set(0.9, 0.9);
								bg.active = false;
								add(bg);
								...
								stageFront.loadGraphic(Paths.image('morningFront'));
								stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
								stageFront.updateHitbox();
								stageFront.antialiasing = true;
								stageFront.scrollFactor.set(0.9, 0.9);
							...
							case 'sunset':
								{
									defaultCamZoom = 0.9;
									curStage = 'sunset';
									var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('sunsetBack'));
									bg.antialiasing = true;
									bg.scrollFactor.set(0.9, 0.9);
									bg.active = false;
									add(bg);
									...
									var stageFront:FlxSprite = new FlxSprite(-650, 600);
									stageFront.loadGraphic(Paths.image('sunsetFront'));
									stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
									stageFront.updateHitbox();
									stageFront.antialiasing = true;
									stageFront.scrollFactor.set(0.9, 0.9);
		...
		override public function update(elapsed:Float)
		{
			...
			if (wireStage) {
				glitch.loadGraphic(Paths.image('WIREStageBack'));
				FlxG.camera.shake(0.005);
			}
			if (!wireStage) {
				glitch.loadGraphic(Paths.image('glitcherBack'));
			}
			...
			else
			{
				...
				// Conductor.lastSongPos = FlxG.sound.music.time;
							...
							// HARDCODING FOR HEX LOL
							/*
							if (curSong == 'Glitcher') {
								trace("Please work, I like this animation");
								if (curBeat > 576) {
									glitch.loadGraphic(Paths.image('WIREStageBack'));
								}
							}
		...
		private function keyShit():Void // I've invested in emma stocks
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				if (PlayStateChangeables.useDownscroll && daNote.y > strumLine.y || !PlayStateChangeables.useDownscroll && daNote.y < strumLine.y)
				{
					...
					if (PlayStateChangeables.botPlay && daNote.canBeHit && daNote.mustPress || PlayStateChangeables.botPlay && daNote.tooLate
						&& daNote.mustPress)
					{
						if (loadRep)
						{
		...
		override function stepHit()
		{
			...
			if (curSong == 'Glitcher') {
				if (curStep == 576 || curStep == 1087) {
					FlxG.camera.flash(FlxColor.WHITE, 0.75);
					wireStage = true;
				}
				if (curStep == 847 || curStep == 1343) {
					FlxG.camera.flash(FlxColor.WHITE, 0.75);
					wireStage = false;
				}
			}
